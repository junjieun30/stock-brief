...
- **새벽 6시 10분 (화~토)** — 미국장 마감 직후. 그날의 메인 브리핑
  (겨울엔 미국장이 한국시간 06:00 정각에 끝나서, 마감 데이터가 확정되도록 10분 뒤에 돕니다)
- **오후 6시 (월~금)** — 저녁 업데이트. 새 뉴스와 아시아·유럽 시황 반영
...
```yaml
- cron: "10 21 * * 1-5"  # 새벽 6시 10분 (화~토)
- cron: "0 9 * * 1-5"    # 오후 6시 (월~금)
```
...
| 원하는 한국시간 | cron 값 |
|---|---|
| 새벽 6시 10분 (화~토) | `10 21 * * 1-5` (현재) |
| 오전 7시 (화~토) | `0 22 * * 1-5` |
| 오후 6시 (월~금) | `0 9 * * 1-5` (현재) |
| 오후 9시 (월~금) | `0 12 * * 1-5` |
