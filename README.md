...
| 오늘의 사색 | 미래학자 20인의 생각을 하루 한 편씩 (명상하듯 읽는 마무리) |
...
**오늘의 사색** — 아마라·토플러·사이먼·깁슨·메도스·브랜드·탈레브·카너먼·커즈와일·켈리 등
미래를 오래 들여다본 20인의 생각을 하루 한 편씩 보여줍니다. `briefing/data/futurists.py`.

> 미래학자 명언은 오귀속이 유난히 많습니다. 그래서 출처가 분명한 것만 인용부호를 붙여
> **인용**으로 표기하고, 나머지는 인용부호 없이 **사상 요약**으로 씁니다. 널리 잘못 알려진
> 문구(토플러↔저저이, 매클루언↔컬킨, 드러커↔앨런 케이)는 카드 아래에 실제 출처를 밝힙니다.

전체 20장을 한 페이지에서 보려면:

```bash
python _preview_reflect.py
```
...
briefing/
  ...
  data/futurists.py           미래학자 20인 · 출처 표기 규칙 포함
```
