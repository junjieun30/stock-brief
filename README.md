...
| 내일의 기술 | 미래 기술 25편을 하루 한 챕터씩 — 각 편에서 장문판 책으로 이어짐 |
...
**『지금에 모인 미래』** — 위 25편의 장문판을 담은 독립된 책 웹앱입니다 (`/book/`).
표지·목차·장별 페이지·이어읽기(기기별 localStorage)·읽음 표시·진행 바를 갖췄고,
브리핑의 그날 챕터에서 "전체 버전 읽기"로 이어집니다. 콘텐츠는 `bookgen/content_p*.py`,
생성기는 `bookgen/build.py`. 다시 만들려면:
